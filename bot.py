...
admin_id = 1366787718868242482
ADMIN_IDS = ["1366787718868242482"]
user_admins = set()
...
ADMIN_IDS = input("        1366787718868242482: ").split(",")
...
def has_permission(interaction: discord.Interaction) -> bool:
    """Kiểm tra nếu user là admin chính hoặc admin phụ"""
...
def has_permission(interaction: discord.Interaction) -> bool:
    """Kiểm tra quyền chung: admin chính hoặc admin phụ"""
...
@tree.command(name="nhaynamebox", description="Treo đổi tên box Messenger ")
@app_commands.describe(
...
async def treomess(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description="Ấn nút Start để điền thông tin.",
        color=0x2ecc71
    )
...
async def addadmin(interaction: discord.Interaction, user: str, thoihan: str = None):
    if not is_owner(interaction):
        embed = discord.Embed(
            title="🚫 Đĩ mẹ m đ/é/o có quyền add.",
            description="Chỉ 𝓨𝓾𝓸𝓷𝓰𝓚💢💤 Mới thêm được.",
            color=discord.Color.red()
        )
        return await interaction.response.send_message(embed=embed)
...
async def xoaadmin(interaction: discord.Interaction, user: str):
    if not is_owner(interaction):
        embed = discord.Embed(
            title="🚫 Bạn nghĩ bạn đủ tư cách hả",
            description="Chỉ **admin gốc** mới được xóa admin phụ.",
            color=discord.Color.red()
        )
        return await interaction.response.send_message(embed=embed)
...
async def on_ready():
    await bot.tree.sync()
    print(f"✅ Bot {bot.user} đã dậy theo lệnh của chủ nhân Van Khang")
...
class AllDisModalCustom(discord.ui.Modal):
    def __init__(self, messages, title="All Discord"):
        ...
        self.mention_ids = discord.ui.TextInput(
            label="⚡ID Người cần tag (Không bắt buộc)", 
            required=False
        )
...
async def tabsettheme(interaction: discord.Interaction, stop: str = None):
    ...
    if stop:
        ...
        try:
            ...
            embed = discord.Embed(
                title="⚠️ Lỗi",
                description=f"Vui lòng nhập số hợp lệ hoặc 'all'.\nChi tiết: `{e}`",
                color=discord.Color.orange()
            )
            return await interaction.response.send_message(embed=embed)
...
class TreoPollModal(discord.ui.Modal, title="Treo Poll Messenger"):
    ...
    box_id = discord.ui.TextInput(label="ID Box", placeholder="", required=True)
    delay = discord.ui.TextInput(label="Delay", placeholder="", default="", required=True)
...
@tree.command(name="tabtreopollmess", description="dừng treopollmes")
@app_commands.describe(stop="Nhập số task để dừng (ví dụ: 1) hoặc 'all' để dừng tất cả")
async def tabtreopollmess(interaction: discord.Interaction, stop: str = None):
    ...
    if stop:
        if stop.lower() == "all":
            for task in tasks:
                ...
                if folder:
                    stop_path = os.path.join(folder, "stop.txt")
    ...
    desc = ""
    for idx, task in enumerate(tasks, 1):
        desc += f"**{idx}.** Box `{task.get('box_id', '?')}` | Delay: {task.get('delay', '?')}s\n"
...
def run_poll(cookie, box_id, delay, user_id, task_key):
    ...
    try:
        ...
        print(f"[run_poll] Lỗi tạo thư mục: {e}")
...
def start_nhay_poll_func(cookie, idbox, delay, folder_id):
    ...
    os.makedirs(folder_path, exist_ok=True)
    stop_file = os.path.join(folder_path, "stop.txt")
    try:
        with open("Vkhangg.txt", "r", encoding="utf-8") as f:
            lines = [line.strip() for line in f if line.strip()]
    except FileNotFoundError:
        print("❌ Không tìm thấy file chui.txt")
        return

    if len(lines) < 3:
        print("❌ File phải có ít nhất 3 dòng.")
        return
    ...
    for i in range(0, len(lines), 3):
        chunk = lines[i:i+3]
        if len(chunk) == 3:
            question, opt1, opt2 = chunk
            polls.append((question, opt1, opt2))

    if not polls:
        print("❌ Không có file chui.txt.")
        return

    user_id = cookie.split("c_user=")[1].split(";")[0]
    print(f"[TreoPoll] Bắt đầu spam poll vào box {idbox} (delay {delay}s)")
    print(f"[TreoPoll] Tổng số poll: {len(polls)}")
    ...
    while True:
        ...
        try:
            question_text, opt1, opt2 = polls[index % len(polls)]
            index += 1
            ...
            if res.status_code == 200:
                print(f"[✓] Poll gửi thành công: {question_text} → {opt1}/{opt2}")
            else:
                print(f"[×] Lỗi ({res.status_code}): {res.text[:120]}")
...
class ComboMessModal(discord.ui.Modal, title="ComboMess"):
    cookie = discord.ui.TextInput(label="Cookie", style=discord.TextStyle.paragraph, required=True)
    box_id = discord.ui.TextInput(label="ID Box", placeholder="Nhập ID box", required=True)
    delay = discord.ui.TextInput(label="Delay (giây)", placeholder="VD: 10", default="10", required=False)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            cookie = str(self.cookie.value).replace("\n", "").strip()
            box_id = re.sub(r"[^\d]", "", str(self.box_id.value))
            delay = float(self.delay.value) if self.delay.value else 10.0
...
async def tabcombomess(interaction: discord.Interaction, stop: str = None):
    ...
    if stop and stop.lower() == "all":
        for task in list(tasks):
            key = task["task_key"]
            STOP_FLAGS[key] = True

            # Nếu có client theme đang mở thì ngắt
            if "THEME_CLIENTS" in globals() and key in THEME_CLIENTS:
                try:
                    THEME_CLIENTS[key].disconnect()
                except:
                    pass
                THEME_CLIENTS.pop(key, None)

            # Xóa folder data nếu có
            folder_path = os.path.join("data", f"combo_{key}")
            if os.path.exists(folder_path):
                try:
                    shutil.rmtree(folder_path)
                    print(f"[Stop] Xóa folder {folder_path}")
                except Exception as e:
    ...
    if stop:
        try:
            ...
            if 0 <= num < len(tasks):
                ...
                USER_COMBO_TASKS[user_id] = tasks
...
class MenuSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="Gmail", value="gmail", description="Các lệnh spam/dừng trên Gmail", emoji="✉️"),
        ...
        super().__init__(
            placeholder="⬇️ Chọn danh mục để xem lệnh của bot...",
...
class MenuSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="🛠️ Quản lí", value="quanli"),
            discord.SelectOption(label="💬 Messenger", value="messenger"),
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if value == "messenger":
            desc = (
                "```"
                "/treoanhmess - spam ảnh mess\n"
                "/treomess - treo ngôn mess\n"
                "/nhaymess - nhây réo tên\n"
                "/nhaytagmess - nhây tag mess\n"
                "/nhaynamebox - nhây tên box\n"
                "/tabnhaymess - dừng tab nhây réo tên\n"
                "/tabnhaytagmess - dừng tab nhây tag mess\n"
                "/tabnhaynamebox- dừng tab nhây name box\n"
                "/setnenmess - đổi nền mess\n"
                "/tabsetnenmess - dừng tab đổi nền mess\n"
                "/nhaypollmess - spam poll mess\n"
                "/tabnhaypollmess - dừng tab treo poll mess\n"
                "/combomess - tạo combo mess siêu bá(chỉ admin)\n"
                "/tabcombomess - dừng tab combo mess\n"
                "/namechange - đổi tên thành viên mess\n"
                "/tabnamechange - dừng tab name change\n"
                "/raidbox - tag pr5 (đang lỗi api)"
                "```"
            )
            ...
            desc = (
                ...
                "/allnhaydis - all các chức năng nhây discord\n"
                "/tabtreodis - tab treo ngôn discord\n"
                "/taballnhaydis - dừng all chức năng nhây discord\n"
                "/treoaudio - spam audio BuiTuanManh.mp3\n"
                "/tabtreoaudio - dừng tab treo audio"
                "```"
            )
            ...
            desc = (
                "```"
                "/menu - hiển thị danh sách chức năng của bot\n"
...
class MenuView(discord.ui.View):
    def __init__(self):
        ...
        self.add_item(discord.ui.Button(
            ...
            url="https://zyo.lol/yuongk"
        ))

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.data.get("custom_id") == "info_bot":
            ...
            desc = (
                ...
                f"• **Tác giả:** 𝓨𝓾𝓸𝓷𝓰𝓚💢💤\n"
                f"• **Version:** `2.0`\n"
                f"• **Ngôn ngữ:** Python · discord.py\n"
                f"• **Ngày ra mắt:** 27/12/2025\n"
                "━━━━━━━━━━━━━━━━━━\n\n"

                "### 💸 **Bảng Giá Dịch Vụ**\n"
                "**Thuê Bot:**\n"
                "› 20k/tuần · 50k/tháng · 100k/năm · 200k/vĩnh viễn\n\n"
                "**Full Source:**\n"
                "› 400k (kèm hướng dẫn + tối ưu)\n"
                "━━━━━━━━━━━━━━━━━━\n\n"

                f"📩 **Liên hệ admin:** <@{1366787718868242482}>"
            )

            info = discord.Embed(
                title="📊 THÔNG TIN BOT",
                description=desc,
                color=discord.Color.from_rgb(30, 140, 255)
            )

            if admin_user and admin_user.avatar:
                info.set_footer(
                    text="𝓨𝓾𝓸𝓷𝓰𝓚💢💤 • Premium Bot System",
                    icon_url=admin_user.avatar.url
                )
            else:
                info.set_footer(text="𝓨𝓾𝓸𝓷𝓰𝓚💢💤 • Premium Bot System")
...
async def menu(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="⚙️ 𝓨𝓾𝓸𝓷𝓰𝓚💢💤 – MENU",
        description=(
            "💀 **Dominion Protocol | 𝓨𝓾𝓸𝓷𝓰𝓚💢💤 System**\n"
            "Chọn danh mục bên dưới để xem lệnh chi tiết."
        ),
        color=discord.Color.from_rgb(40, 40, 40)
    )

    embed.set_thumbnail(
        url="https://media.discordapp.net/attachments/1402549122803961856/1435610005415006278/4712027.png?ex=69337bc1&is=69322a41&hm=1e0fded25e409bec1c1e9bc9451e494b59aade73f3eb74fcaea23b147bc759d3&=&format=webp&quality=lossless"
    )
    embed.set_footer(text="𝓨𝓾𝓸𝓷𝓰𝓚💢💤 Bot")
...
bot.run(TOKEN) title="📊 THÔNG TIN BOT",
                description=desc,
                color=discord.Color.from_rgb(30, 140, 255)
            )
